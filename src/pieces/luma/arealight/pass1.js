import { ShaderPass } from '@/pieces/luma/common/pass'
import { constantValue } from '@/pieces/luma/common/utils'
import { Matrix4 } from 'math.gl'

export default function getPass(props) {
  return new ShaderPass({
    ...
    fs: `
          ...
          vec3 N, vec3 V, vec3 P, mat3 Minv, vec3 points[4], int twoSided)
      {
          ...
          sum = twoSided == 1 ? abs(sum) : max(0.0, sum);
    ...
    render({ gl, model, startTime, framebuffer }) {
      gl.viewport(0, 0, gl.drawingBufferWidth, gl.drawingBufferHeight)
  
      model.uniforms.u_time = Date.now() - startTime
      model.uniforms.resolution = [gl.drawingBufferWidth, gl.drawingBufferHeight]
  
      model.uniforms.roughness = parseFloat(props.roughness)
      model.uniforms.dcolor = JSON.parse(props.dcolor)
      model.uniforms.scolor = JSON.parse(props.scolor)
      model.uniforms.intensity = parseFloat(props.intensity)
      model.uniforms.height = parseFloat(props.height)
      model.uniforms.width = parseFloat(props.width)
      model.uniforms.roty = parseFloat(props.roty)
      model.uniforms.rotz = parseFloat(props.rotz)
      model.uniforms.twoSided = parseInt(props.twoSided)
      if (!model.uniforms.view) {
        model.uniforms.view = new Matrix4()
        model.uniforms.view.translate([0, 6, -0.5])
        console.log(model.uniforms.view)
        // model.uniforms.view // rotate(-10.0, 1, 0, 0);
        // model.uniforms.view.rotate(0, 0, 1, 0);
      }
